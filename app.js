...
async function loadTicketsFromSupabase() { const { data, error } = await supabaseClient .from('tickets') .select('*'); console.log('📦 Datos crudos desde Supabase:', data); if (error) { console.error('❌ Error cargando tickets:', error); return; } // resto del código... }
